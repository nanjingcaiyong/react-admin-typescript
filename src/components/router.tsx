  ...
   recombination(item:IRoute, i:number):JSX.Element{
    ...
    return (
     /**
       ...
       */
      <Route exact={ !hasChildren } key={ i } path={ item.path } strict={ true } render={
        props => 
          <Suspense fallback={<div>正在加载...</div>}>
            {
              // 判断是否具有子元素
              hasChildren ? (item.children || []).map((t:IRoute, j:number)=> (<item.component key={ j }>{this.recombination(t,j)}</item.component>))
